...
**AI auto-fix PR:** enable auto-fix (web UI toggle, or the auto-fix path with a `--github-token`) together with an LLM key, and SecureScope opens a pull request that applies **real code patches** generated by the model for each fixable finding. Each patch passes a conservative sanity check (non-empty, similar size, actually changed); anything the model can't fix safely falls back to an inline `[SecureScope Fix]` suggestion comment. Always review the PR before merging.

### Example 2b — Compliance evidence pack (SOC 2 / ISO 27001)

```powershell
python main.py --repo https://github.com/owner/repo `
  --no-advisor --no-sandbox --compliance --evidence-pack `
  --out-dir ./reports
```

Writes `{repo}_{ts}_evidence.html` — an auditor-style control matrix mapping the scan's OWASP-classified findings to **SOC 2 Trust Services Criteria** and **ISO/IEC 27001:2022 Annex A** controls. Each control is reported as *No exceptions noted* or *Exception(s) noted* with the finding count and driving OWASP categories. The web app generates the same pack automatically on each scan and links it from the results. **Screenshot:** `docs/screenshots/19_evidence_pack.png`
