...
GitHub Repo URL
  ...
  main.py (orchestrator)
      │
  ┌───┴──────────────────────────────────────────────────────────────┐
  │  analyzer.py      sandbox.py        dependency_scanner.py        │
  │  (Semgrep SAST)   (Docker + strace) (pip-audit → OSV.dev)       │
  │                                                                   │
  │  secret_scanner.py    iac_scanner.py       trivy_scanner.py      │
  │  (detect-secrets +    (checkov + 50+       (container CVE scan)  │
  │   entropy detection)   built-in patterns)                        │
  │                                                                   │
  │  polyglot_scanner.py  dast.py              supply_chain.py       │
  │  (npm/cargo/go/gem/   (Nuclei → ZAP        (dependency confusion │
  │   maven/packagist)     fallback)            + typosquatting)     │
  └──────────────────────────────────────────────────────────────────┘
      │
  advisor.py (AI fix generation — Claude / GPT / Gemini / Groq / Ollama)
      │
  ┌───┴──────────────────────────────────────────────────────────────────┐
  │  report.py          sarif.py       sbom.py       compliance.py       │
  │  (HTML + JSON)      (SARIF 2.1.0)  (CycloneDX)   (PCI/NIST/OWASP)  │
  │                                                                       │
  │  markdown_report.py   pdf_report.py   db.py          trend.py        │
  │  (PR comments)        (weasyprint)    (SQLite store)  (sparkline)    │
  └──────────────────────────────────────────────────────────────────────┘
      │
  ┌───┴─────────────────────────────────────────────────────────────┐
  │  GitHub Issues    Jira Cloud    Slack / Teams    PR comment      │
  │  (auto-create)    (Bug tickets) (webhook card)   (markdown post) │
  └─────────────────────────────────────────────────────────────────┘
```
...
│  medium      INTEGER                                                │
│  low         INTEGER                                                │
│  sarif_path  TEXT (nullable)                                        │
│  sbom_path   TEXT (nullable)                                        │
└─────────────────────────────────────────────────────────────────────┘
...
│  cwe         TEXT                                                   │
│  message     TEXT                                                   │
│  status      TEXT (open / fixed / suppressed)                       │
└─────────────────────────────────────────────────────────────────────┘
...
│  days_open   INTEGER                                                │
│  breached    INTEGER (0 = ok, 1 = SLA breached)                     │
└─────────────────────────────────────────────────────────────────────┘
```
